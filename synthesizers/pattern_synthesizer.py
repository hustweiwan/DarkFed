...
class PatternSynthesizer(Synthesizer):
    ...
    def synthesize_inputs(self, batch, attack_portion=None):
        ...
        #为1的位置直接变成0后再放上pattern
        # if len(batch.inputs)!=attack_portion:
        #     attack_portion*=1.5
        batch.inputs[:round(attack_portion)] = (1 - mask) * \
                                        batch.inputs[:round(attack_portion)] + \
                                        mask * pattern
        # batch.inputs = (1 - mask) * \
        #                                        batch.inputs + \
        #                                        mask * pattern
    ...
    def synthesize_labels(self, batch, attack_portion=None):
        ...
        #batch.labels[batch.labels == 1].fill_(self.params.backdoor_label)
        return

    def get_pattern(self):
        #这个函数应该是用来动态调整mask和patter在整个图片中的位置
        #这里pattern可能会翻转并且改变大小，而且x，y的位置会改变
